...
func (self *Token) Equal(b *Token) bool {
	if self == nil || b == nil {
		return false
	}
	if self.tokenType != b.tokenType {
		return false
	}
...
func (self *TokenClassifier) ClassifyRune(rune int32) RuneTokenType {
	return self.typeMap[rune]
}
...
func (self *Lexer) NextWord() (string, error) {
	var token *Token
	var err error
	for {
		token, err = self.tokenizer.NextToken()
		if err != nil {
			return "", err
		}
...
func (self *Tokenizer) NextToken() (*Token, error) {
