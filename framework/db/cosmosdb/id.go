...
func readMachineID() []byte {
	...
	if err == nil && len(hid) != 0 {
		...
		if _, randErr := rand.Reader.Read(id); randErr != nil {
			panic(fmt.Errorf("[error] cannot get hostname nor generate a random number: %v; %v", err, randErr))
		}
	}
	return id
}
...
func randInt() uint32 {
	...
	if _, err := rand.Reader.Read(b); err != nil {
		panic(fmt.Errorf("[error] cannot generate random number: %v;", err))
	}
...
func (id *ID) Scan(value interface{}) (err error) {
	...
	case nil:
		*id = nilID
		return nil
	default:
		return fmt.Errorf("[error] scanning unsupported type: %T", value)
	}
}
...
// NilID returns a zero value for `id.ID`.
func NilID() ID {
	return nilID
}
